...
import { extractHeadings, PostHeading } from "../../lib/utils/extract-headings";
...
const RESULTS_ID = "post-search-results";
const optionId = (index: number) => `post-search-option-${index}`;
...
export const PostSearch = (props: Props) => {
  ...
  const handleDialogKeydown = (event: React.KeyboardEvent) => {
    // The input and the close button are the only tab stops (results are
    // picked with the arrow keys), so keep Tab cycling between them instead
    // of escaping to the page behind the dialog.
    if (event.key === "Tab") {
      event.preventDefault();
      const next =
                  ...
                  >
                    {results.map((heading, index) => (
                      <li
                        ...
                        className={`block w-full cursor-pointer pr-4 py-2 text-left text-sm duration-150 ${
                          heading.level === 4
                            ? "pl-10"
                            : heading.level === 3
                              ? "pl-7"
