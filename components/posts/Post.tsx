...
export const Post = (props: Props) => {
  ...
  useEffect(() => {
    if (articleRef.current) {
      ...
      codeBlocks.forEach((code) => {
        ...
        copyBtn.innerHTML =
          '<svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>';
  ...
  // Code blocks that scroll sideways need to be focusable so keyboard users
  // can scroll them with the arrow keys. Only those, so short blocks don't
  // add pointless tab stops. Re-checked on resize as the column narrows.
  useEffect(() => {
    const article = articleRef.current;
    if (!article) return;

    const updateScrollableCode = () => {
      // Includes plain ``` blocks with no language, which highlight.js skips.
      article
        .querySelectorAll<HTMLElement>("pre code:not(.line-numbers)")
        .forEach((code) => {
            ...
            {props.post.finished ? (
              <div className="ml-2 outline-none flex">
                <Link
                  ...
                  aria-label={`Share "${props.post.title}" on X (Twitter)`}
                  target="_blank"
                >
                  <FaXTwitter
